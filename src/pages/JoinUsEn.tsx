...
export default function JoinUsEn() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 1. Validate data
      const validatedData = applicationSchemaEn.parse(rawData);
      ...
      if (error?.issues?.[0]?.message) {
        ...
        setShowError(error.message || "An error occurred while submitting the application. Please try again.");
      }
    } finally {
      setIsSubmitting(false);
    }
                ...
                <div className="p-6 border-t border-white/5 bg-black/20 backdrop-blur-xl relative z-20 flex flex-col gap-5">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <div className="flex gap-1 bg-white/5 p-1 rounded-lg border border-white/10">
                      <button onClick={() => navigate('/ar/join')} className="px-3 py-1.5 rounded-md bg-transparent text-white/40 font-bold text-xs flex items-center justify-center hover:text-white transition-all">
                        AR
                      </button>
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 pt-28 pb-4 md:pt-36 md:pb-8">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="section-subtitle mb-0">Careers</span>
            <div className="w-10 h-[2px] bg-white/10 rounded-full relative overflow-hidden">
              <motion.div 
                animate={{ x: ["-100%", "200%"] }}
          ...
          <h1 className="section-title">Join <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-accent-light">Our Team</span></h1>
          <p className="text-white/50 text-sm md:text-lg max-w-2xl mx-auto font-light leading-relaxed">We are always looking for exceptional talent to join the Effect Media team. Fill out the form below and we will contact you.</p>
        </motion.div>
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs text-white/40 ml-11">Tell us a little about yourself and how we can reach you.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">Country / City</label>
                <div className="relative group">
                  <MapPin size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-accent transition-colors" />
                  <input type="text" name="location" required placeholder="Where do you live?" className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 px-12 text-white placeholder:text-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs text-white/40 ml-11">Help us understand your skills and areas of creativity.</p>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">Primary Area of Expertise</label>
              <div className="relative group">
                <Briefcase size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-accent transition-colors" />
                <select name="expertise" required defaultValue="" className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 px-12 text-white focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all appearance-none cursor-pointer">
                  ...
                  <option value="programming">Programming (Web / Mobile Apps)</option>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">Years of Experience</label>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                {["Less than 1 year", "1-3 years", "More than 3 years"].map((exp) => (
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">Portfolio / Previous Work</label>
              <div className="relative group">
                <LinkIcon size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-accent transition-colors" />
                <input type="url" name="portfolio" required placeholder="Share a link to your work (Behance, Drive, Personal Website, etc.)" className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 px-12 text-white placeholder:text-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all" dir="ltr" />
              </div>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">Tools & Skills</label>
              <div className="relative group">
                <PenTool size={18} className="absolute left-4 top-4 text-white/20 group-focus-within:text-accent transition-colors" />
                <textarea name="skills" required placeholder="What tools or technologies do you use? (e.g., Photoshop, Premiere Pro, Figma, WordPress, Google Ads...)" rows={3} className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 px-12 text-white placeholder:text-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all resize-none"></textarea>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs text-white/40 ml-11">Help us match you with projects that fit your budget.</p>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">Hourly Rate (EGP)</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative flex items-center group">
                  <span className="absolute left-4 text-white/30 text-[10px] md:text-xs font-bold group-focus-within:text-accent transition-colors">Minimum</span>
                  <input type="number" name="min_rate" min="0" required placeholder="0" className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 pl-20 md:pl-24 pr-14 md:pr-16 text-white placeholder:text-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all text-left font-mono text-sm md:text-base" dir="ltr" />
                  <span className="absolute right-4 text-white/30 text-[8px] md:text-[10px] font-bold">EGP/H</span>
                </div>
                <div className="relative flex items-center group">
                  <span className="absolute left-4 text-white/30 text-[10px] md:text-xs font-bold group-focus-within:text-accent transition-colors">Maximum</span>
                  <input type="number" name="max_rate" min="0" required placeholder="0" className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 pl-20 md:pl-24 pr-14 md:pr-16 text-white placeholder:text-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all text-left font-mono text-sm md:text-base" dir="ltr" />
                  <span className="absolute right-4 text-white/30 text-[8px] md:text-[10px] font-bold">EGP/H</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider ml-1">About You</label>
              <div className="relative group">
                <MessageSquare size={18} className="absolute left-4 top-4 text-white/20 group-focus-within:text-accent transition-colors" />
                <textarea name="bio" required placeholder="Briefly describe your experience, skills, and what you can bring to the team..." rows={4} className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 px-12 text-white placeholder:text-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/10 transition-all resize-none"></textarea>
              </div>
            </div>
          </div>
        ...
        {showSuccess && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            >
              ...
              <p className="text-white/50 mb-8 leading-relaxed">Thank you for your interest in joining Effect Media. We have received your application and our team will review it and contact you as soon as possible.</p>
              <button 
                onClick={() => {
                  setShowSuccess(false);
                  window.scrollTo(0, 0);
